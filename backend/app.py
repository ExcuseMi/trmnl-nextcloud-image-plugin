...
async def image():
    ...
    if not (nextcloud_url and username and token):
        return jsonify({'image_url': None, 'image_name': '', 'error': 'Missing nextcloud_url, username, or token'}), 400
    ...
    try:
        ...
        if not selected:
            return jsonify({'image_url': None, 'image_name': '', 'error': 'No images found'})

        image_url = await get_direct_link(nextcloud_url, username, token, selected['file_id'])
        return jsonify({'image_url': image_url, 'image_name': selected['name']})

    except Exception as exc:
        log.exception('Error fetching image')
        return jsonify({'image_url': None, 'image_name': '', 'error': str(exc)}), 500
